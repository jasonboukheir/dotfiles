{
  ...
  homelabCfg = config.homelab.services.ai;
...
in {
  config = lib.mkMerge [
    ...
    (lib.mkIf homelabCfg.enable {
      homelab.ports.allocate.open-webui = 3100;
      ...
      services.pocket-id.ensureClients.open-webui = {
      ...
      age.secrets = {
        "open-webui/openaiApiKey".file = config.homelab.secretsDir + /open-webui/openaiApiKey.age;
        "open-webui/webuiSecretKey".file = config.homelab.secretsDir + /open-webui/webuiSecretKey.age;
      };
