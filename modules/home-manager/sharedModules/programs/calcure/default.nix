{
  ...
  # Merge user settings on top of defaults (user wins)
  mergedSettings = lib.recursiveUpdate defaultSettings cfg.settings;

  configFile =
    lib.generators.toINI {} mergedSettings
    + lib.optionalString (cfg.extraConfig != "") ("\n" + cfg.extraConfig + "\n");
in {
  options.programs.calcure = {
    enable = lib.mkEnableOption "configuration for calcure";

    package = lib.mkOption {
      type = lib.types.package;
      default = pkgs.calcure;
      defaultText = lib.literalExpression "pkgs.calcure";
      description = "The calcure package to install.";
    };
    ...
    settings = lib.mkOption {
      ...
      default = {};
      description = ''
        Settings for calcure's config.ini, structured as
        `{ Section = { key = "value"; }; }`.
        Full defaults are provided; user values override them.
      '';
    ...
    extraConfig = lib.mkOption {
      ...
      default = "";
      description = "Extra text appended verbatim to config.ini.";
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    xdg.configFile."calcure/config.ini".text = configFile;
  };
}
